...
export function registerFactorRoutes() {
  ...
  .add('GET','/admin/signal-quality', async ({ env, req, url }) => { if (!admin(env, req)) return err(ErrorCodes.Forbidden,403); const day = url.searchParams.get('as_of') || new Date().toISOString().slice(0,10); const rs = await env.DB.prepare(`SELECT factor, ic_mean, ic_vol, ic_autocorr_lag1, ic_half_life FROM signal_quality_metrics WHERE as_of = ?`).bind(day).all(); return json({ ok:true, as_of: day, metrics: rs.results||[] }); })
