...
export function registerFactorRoutes() {
  router
    ...
    .add('POST','/admin/factor-returns/run', async ({ env, req }) => { if (!admin(env, req)) return json({ ok:false, error:'forbidden' },403); const out = await computeFactorReturns(env); await audit(env, { actor_type:'admin', action:'run', resource:'factor_returns', resource_id:(out as any).as_of||null }); return json(out); })
    .add('GET','/admin/factor-risk', async ({ env, req, url }) => { if (!admin(env, req)) return json({ ok:false, error:'forbidden' },403); const day = url.searchParams.get('as_of') || new Date().toISOString().slice(0,10); const rs = await env.DB.prepare(`SELECT factor_i, factor_j, cov, corr FROM factor_risk_model WHERE as_of = ?`).bind(day).all(); return json({ ok:true, as_of: day, pairs: rs.results||[] }); })
    .add('GET','/admin/factor-metrics', async ({ env, req, url }) => { if (!admin(env, req)) return json({ ok:false, error:'forbidden' },403); const day = url.searchParams.get('as_of') || new Date().toISOString().slice(0,10); const rs = await env.DB.prepare(`SELECT factor, vol, beta FROM factor_metrics WHERE as_of = ?`).bind(day).all(); return json({ ok:true, as_of: day, metrics: rs.results||[] }); })
    .add('GET','/admin/factor-returns-smoothed', async ({ env, req, url }) => { if (!admin(env, req)) return json({ ok:false, error:'forbidden' },403); const day = url.searchParams.get('as_of') || new Date().toISOString().slice(0,10); const rs = await env.DB.prepare(`SELECT factor, ret_smoothed FROM factor_returns_smoothed WHERE as_of = ?`).bind(day).all(); return json({ ok:true, as_of: day, returns: rs.results||[] }); })
