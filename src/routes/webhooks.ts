...
export function registerWebhookRoutes() {
  ...
  .add('POST','/admin/webhooks/delete', async ({ env, req }) => { if (!admin(env, req)) return json({ ok:false, error:ErrorCodes.Forbidden },403); const body:any = await req.json().catch(()=>({})); const id = (body.id||'').toString(); if (!id) return json({ ok:false, error:'id_required' },400); await env.DB.prepare(`DELETE FROM webhook_endpoints WHERE id=?`).bind(id).run(); await audit(env, { actor_type:'admin', action:'delete', resource:'webhook', resource_id:id }); return json({ ok:true, id }); })
  .add('GET','/admin/webhooks/deliveries', async ({ env, req }) => { if (!admin(env, req)) return json({ ok:false, error:ErrorCodes.Forbidden },403); await env.DB.prepare(`CREATE TABLE IF NOT EXISTS webhook_deliveries (id TEXT PRIMARY KEY, webhook_id TEXT, event TEXT, payload TEXT, ok INTEGER, status INTEGER, error TEXT, created_at TEXT, attempt INTEGER, duration_ms INTEGER, nonce TEXT);`).run(); try { await env.DB.prepare(`ALTER TABLE webhook_deliveries ADD COLUMN nonce TEXT`).run(); } catch {} const rs = await env.DB.prepare(`SELECT id, webhook_id, event, ok, status, error, created_at, attempt, duration_ms, nonce FROM webhook_deliveries ORDER BY created_at DESC LIMIT 200`).all(); return json({ ok:true, rows: rs.results||[] }); })
  .add('POST','/webhooks/email/bounce', async ({ env, req }) => {
      ...
      const type = String(body.type||'bounce').toLowerCase();
  ...
  .add('GET','/admin/email/bounces', async ({ env, req }) => { if (!admin(env, req)) return json({ ok:false, error:ErrorCodes.Forbidden },403); try { await env.DB.prepare(`CREATE TABLE IF NOT EXISTS email_bounces (id TEXT PRIMARY KEY, provider TEXT, message_id TEXT, type TEXT, raw TEXT, created_at TEXT);`).run(); } catch {} const rs = await env.DB.prepare(`SELECT id, provider, message_id, type, created_at FROM email_bounces ORDER BY created_at DESC LIMIT 200`).all(); return json({ ok:true, rows: rs.results||[] }); })
    .add('GET','/admin/webhooks/verify', async ({ env, req, url }) => { if (!admin(env, req)) return json({ ok:false, error:'forbidden' },403); const nonce = (url.searchParams.get('nonce')||'').trim(); if (!nonce) return json({ ok:false, error:'nonce_required' },400); try { await env.DB.prepare(`CREATE TABLE IF NOT EXISTS webhook_deliveries (id TEXT PRIMARY KEY, webhook_id TEXT, event TEXT, payload TEXT, ok INTEGER, status INTEGER, error TEXT, created_at TEXT, attempt INTEGER, duration_ms INTEGER, nonce TEXT);`).run(); } catch {} let seen = false; try { const rs = await env.DB.prepare(`SELECT 1 FROM webhook_deliveries WHERE nonce=? LIMIT 1`).bind(nonce).all(); seen = !!(rs.results && rs.results.length); } catch {} return json({ ok:true, nonce, seen }); });
  ...
  router.add('POST','/admin/webhooks/redeliver', async ({ env, req }) => {
    ...
    if (!deliveryId) return json({ ok:false, error:'delivery_id_required' },400);
  ...
  let wh: any;
  try { wh = await env.DB.prepare(`SELECT id,url,secret,active FROM webhook_endpoints WHERE id=?`).bind(row.webhook_id).all(); } catch (e:any) { return json({ ok:false, error:'webhook_lookup_failed', details:String(e) },500); }
  const whRow: any = wh.results?.[0];
    if (!whRow || !whRow.active) return json({ ok:false, error:'webhook_inactive' },400);
  ...
  } catch (e:any) { return json({ ok:false, error:'insert_failed', details:String(e) },500); }
  ...
  router.add('POST','/webhooks/inbound', async ({ env, req }) => {
    ...
    if (!env.INBOUND_WEBHOOK_SECRET) return json({ ok:false, error:'disabled' },404);
    const bodyText = await req.text();
    let payload: any = null;
    try { payload = JSON.parse(bodyText); } catch { return json({ ok:false, error:'invalid_json' },400); }
    const sig = req.headers.get('x-signature')||'';
    const tsStr = req.headers.get('x-signature-ts')||'';
    const nonce = req.headers.get('x-signature-nonce')||'';
    if (!sig || !tsStr || !nonce) return json({ ok:false, error:'missing_signature' },400);
    const ts = parseInt(tsStr,10);
    if (!Number.isFinite(ts)) return json({ ok:false, error:'invalid_ts' },400);
    const nowSec = Math.floor(Date.now()/1000);
    if (Math.abs(nowSec - ts) > 300) return json({ ok:false, error:'stale' },400); // >5m window
    // Replay protection
    ...
    if (existing.results && existing.results.length) return json({ ok:false, error:'replay' },409);
    // Verify HMAC
    try {
      ...
      if (expected !== sig) return json({ ok:false, error:'bad_signature' },401);
    } catch { return json({ ok:false, error:'sig_verify_error' },500); }
