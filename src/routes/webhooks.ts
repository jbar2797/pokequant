...
export function registerWebhookRoutes() {
  ...
  .add('GET','/admin/webhooks/deliveries', async ({ env, req, url }) => { if (!admin(env, req)) return json({ ok:false, error:ErrorCodes.Forbidden },403); await env.DB.prepare(`CREATE TABLE IF NOT EXISTS webhook_deliveries (id TEXT PRIMARY KEY, webhook_id TEXT, event TEXT, payload TEXT, ok INTEGER, status INTEGER, error TEXT, created_at TEXT, attempt INTEGER, duration_ms INTEGER, nonce TEXT);`).run(); try { await env.DB.prepare(`ALTER TABLE webhook_deliveries ADD COLUMN nonce TEXT`).run(); } catch {} try { await env.DB.prepare(`ALTER TABLE webhook_deliveries ADD COLUMN signature TEXT`).run(); } catch {} try { await env.DB.prepare(`ALTER TABLE webhook_deliveries ADD COLUMN sig_ts INTEGER`).run(); } catch {} // Always include payload so downstream signature verification tests can parse canonical JSON without needing an include flag.
    const rs = await env.DB.prepare(`SELECT id, webhook_id, event, payload, ok, status, error, created_at, attempt, duration_ms, nonce, signature, sig_ts FROM webhook_deliveries ORDER BY created_at DESC LIMIT 200`).all(); return json({ ok:true, rows: rs.results||[], include_payload: true }); })
