...
router.add('POST','/admin/test/metric/insert', async ({ env, req }) => {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(d)) return err(ErrorCodes.InvalidDate, 400);
  if (!metric) return err(ErrorCodes.MetricRequired, 400);
...
router.add('GET','/admin/test/metric/exists', async ({ env, req, url }) => {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(d) || !metric) return err(ErrorCodes.InvalidParams, 400);
