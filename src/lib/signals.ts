...
// Simple signal component + signal generation (lightweight; replaces removed legacy bulk compute)
// Design goals:
//  - Deterministic & fast (suitable for FAST_TESTS)
//  - Idempotent per (card_id, as_of)
//  - Provides enough factor/component fields for downstream factor IC / returns tests

interface PriceRow { card_id: string; as_of: string; price_usd: number }

function linearSlope(rows: PriceRow[]): number|null {
  if (rows.length < 3) return null;
  // x = 0..n-1 chronological
...
function madLogRet(rows: PriceRow[]): number|null {
  ...
  const med = rets.slice().sort((a,b)=>a-b)[Math.floor(rets.length/2)];
  const devs = rets.map(r=> Math.abs(r-med));
  const mad = devs.sort((a,b)=>a-b)[Math.floor(devs.length/2)];
  return mad;
}

export async function computeAndStoreSignals(env: Env, opts?: { limit?: number }) {
  ...
  // Select candidate cards (some price history)
  ...
  for (const c of cards) {
    ...
    // Pull last 90 days (or fewer) of prices
    ...
    const latestDate = latest.as_of;
    if (latestDate !== today) continue; // only compute after price for today present
    idsProcessed++;
    // Slice windows
    ...
    // Drawdown from peak within window (use all)
    let peak = -Infinity; let dd = 0; for (const r of prices) { if (r.price_usd > peak) peak = r.price_usd; }
    if (peak > 0) dd = (peak - latest.price_usd)/peak; // 0 good, >0 drawdown
    const vol = madLogRet(last30);
    // Simplified stand-ins for additional research columns
    const liquidity = prices.length; // proxy: more days -> more liquidity
    const scarcity = 1/Math.max(1, prices.length); // inverse of liquidity
    const mom90 = slope30 ?? slope7 ?? 0;
    const z_svi = 0; // placeholder (no svi transform here)
    // Score: scaled combination (normalize by latest price to keep small)
    ...
    const edge_z = score; // treat score as z for placeholder
    const exp_ret = score/10; // arbitrary scaling
    const exp_sd = volAdj*5; // scaled MAD
    const reasons = [] as string[];
    if (signal==='BUY') reasons.push('positive_trend');
    else if (signal==='SELL') reasons.push('negative_trend');
    if (dd>0.2) reasons.push('drawdown');
    try {
  ...
  return { idsProcessed, wroteSignals };
}
